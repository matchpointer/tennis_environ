...
from surf import Hard, Clay, Carpet, Grass
import tennis
...
class TourAverageSetProcessor(object):
    ...
    def reporting(self):
        ...
        for level in (lev.masters, lev.gs, lev.main, lev.chal):
            for surface in (Hard, Clay, Carpet, Grass):
                res_list = []
                for (srf, lvl, cou, tour_name) in \
                        self.as_years_nt_nm_from_surf_lev_cou_tname.keys():
                    if (srf != surface) or (lvl != level):
                        continue
                    avgs_years_nt_nm = self.as_years_nt_nm_from_surf_lev_cou_tname[
                        (srf, lvl, cou, tour_name)
                    ]
...
class DrawStatusProcessor(object):
    ...
    def __lucky_looser_process(self, tour, match, qual_rounds):
        ...
        if not qual_rounds or match.rnd != "First":
            return
    ...
    def __sets_process(self, tour, match):
        ...
        if match.rnd == "First" and "qual" in (match.first_draw_status, match.second_draw_status):
        ...
        elif match.rnd == "Second" and "bye" in (match.first_draw_status, match.second_draw_status):
    ...
    def __process_match(self, tour, match, qual_rounds):
        ...
        if (
            match.rnd not in ("Second", "First")
            or match.first_draw_status in ("qual", "bye")
            or match.second_draw_status in ("qual", "bye")
        ):
            return  # already done?
...
def do_stat():
    try:
        ...
        process([total.PlayersTotalProcessor(sex=args.sex)],
                sex=args.sex, min_date=min_date, time_reverse=False)
